...
const interestExpenseSchema = z.object({
  ...
  vextir: z.string().regex(/^\d+(\.\d+)?$/, { message: 'Upphæð verður að vera tala' }),
...
const InterestExpensesForm: React.FC<InterestExpensesFormProps> = ({ onNext, initialData, onBack }) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
    ...
    defaultValues: initialData || {
      interestExpenses: [
        { lánshluti: 'Húsnæðislán', vextir: '1.200.000', dagsetning: '2024-01-01' }
      ]
    }
  })
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Stack space={4}>
            ...
            <Box key={field.id} marginY={2}>
              <GridRow>
                <GridColumn span={['12/12', '6/12', '6/12']}>
                  <Box marginBottom={1}>
                    <Text variant="small" color="blue400">Lánshluti</Text>
                  </Box>
                  <Controller
                    ...
                    render={({ field: { onChange, value, ...field } }) => {
                      ...
                      return (
                        <Select
                          ...
                          options={options}
                          hasError={!!errors.interestExpenses?.[index]?.lánshluti}
                          errorMessage={errors.interestExpenses?.[index]?.lánshluti?.message}
                ...
                <GridColumn span={['12/12', '6/12', '6/12']}>
                  <Box marginBottom={1}>
                    <Text variant="small" color="blue400">Vextir</Text>
                  </Box>
                  <Controller
                    ...
                    render={({ field }) => (
                      <Input
                        ...
                        errorMessage={errors.interestExpenses?.[index]?.vextir?.message}
                        required
                      />
                    )}
                  />
              ...
              <Box marginTop={2}>
                <Box marginBottom={1}>
                  <Text variant="small" color="blue400">Dagsetning</Text>
                </Box>
                <Controller
                  ...
                  render={({ field }) => (
                    <DatePicker
                      ...
                      errorMessage={errors.interestExpenses?.[index]?.dagsetning?.message}
          ...
          <Box display="flex" justifyContent="flexEnd" marginTop={2}>
            <Button
              variant="ghost"
              onClick={() => append({ lánshluti: '', vextir: '', dagsetning: '' })}
              icon="add"
              iconType="outline"
              size="small"
            >
