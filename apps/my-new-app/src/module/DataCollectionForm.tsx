...
const formSchema = z.object({
  nafn: z.string().min(2, { message: 'Nafn þarf að vera að minnsta kosti 2 stafir' }),
  kennitala: z
    .string()
  ...
  heimilisfang: z.string().min(5, { message: 'Heimilisfang þarf að vera að minnsta kosti 5 stafir' }),
});
...
const DataCollectionForm: React.FC<DataCollectionFormProps> = ({ onNext, initialData, onBack }) => {
  const {
    control,
    handleSubmit,
  ...
  return (
    <Box className='w-full' padding={4} background="blue100" borderRadius="large">
      <form onSubmit={handleSubmit(onSubmit)}>
        <Stack space={3}>
          <Box marginBottom={3}>
            <Text variant="h2" as="h1">Skráningarform</Text>
          </Box>
