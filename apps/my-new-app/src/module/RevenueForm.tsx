...
const incomeItemSchema = z.object({
  source: z.string().min(1, { message: 'Vinsamlegast fylltu út uppruna tekna' }),
  amount: z.string().regex(/^\d+(\.\d+)?$/, { message: 'Upphæð verður að vera tala' })
});
...
const RevenueForm: React.FC<RevenueFormProps> = ({ onNext, initialData, onBack }) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
    ...
    defaultValues: initialData || {
      ...
      subsidyItems: [
        { type: 'Ferðaskrifstofan', amount: '900.000' }
      ]
    }
  });
  ...
  return (
    <Box>
      <form onSubmit={handleSubmit(onSubmit)}>
        <Box marginBottom={4}>
          <Text variant="h3" as="h2">Launatekjur og starfstengdar greiðslur</Text>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Input
                        ...
                        errorMessage={errors.incomeItems?.[index]?.source?.message}
                        required
                      />
                    )}
                  />
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Input
                        ...
                        errorMessage={errors.incomeItems?.[index]?.amount?.message}
                        required
                      />
                    )}
                  />
        ...
        <Box marginY={4}>
          <Text variant="h3" as="h2">Ökutækjastyrkur. Dagpeningar. Hlunnindi</Text>
          ...
          {subsidyFields.map((field, index) => (
            <Box key={field.id} marginY={2}>
              <GridRow>
                <GridColumn span={['12/12', '12/12', '6/12']}>
                  <Box marginBottom={1}>
                    <Text variant="small" color="blue400">Tegund fyrirtækis</Text>
                  </Box>
                  <Controller
                    ...
                    render={({ field: { onChange, value, ...field } }) => (
                      <Select
                        ...
                        errorMessage={errors.subsidyItems?.[index]?.type?.message}
                        value={subsidyOptions.find(option => option.value === value) || null}
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Input
                        ...
                        errorMessage={errors.subsidyItems?.[index]?.amount?.message}
                        required
                      />
                    )}
                  />
