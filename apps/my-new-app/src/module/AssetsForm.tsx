...
const realEstateSchema = z.object({
  ...
  fasteign_mat: z.string().regex(/^\d+(\.\d+)?$/, { message: 'Upphæð verður að vera tala' })
});

const vehicleSchema = z.object({
  numer: z.string().min(1, { message: 'Skráningarnúmer er nauðsynlegt' }),
  kaupar: z.string().regex(/^\d{4}$/, { message: 'Ártal verður að vera 4 tölustafir' }),
  kaupverd: z.string().regex(/^\d+(\.\d+)?$/, { message: 'Upphæð verður að vera tala' })
});
...
const AssetsForm: React.FC<AssetsFormProps> = ({ onNext, initialData, onBack }) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
    ...
    defaultValues: initialData || {
      realEstate: [
        { fastanumer: '210-9876', heimilisfang: 'Bláfjallagata 12', fasteign_mat: '52.000.000' }
      ],
      vehicles: [
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Box marginBottom={4}>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Input
                        ...
                        errorMessage={errors.realEstate?.[index]?.fastanumer?.message}
                        required
                      />
                    )}
                  />
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Input
                        ...
                        errorMessage={errors.realEstate?.[index]?.heimilisfang?.message}
                        required
                      />
                    )}
                  />
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Input
                      ...
                      errorMessage={errors.realEstate?.[index]?.fasteign_mat?.message}
                      required
                    />
                  )}
                />
          ...
          <Box display="flex" justifyContent="flexEnd" marginTop={2}>
            <Button
              variant="ghost"
              onClick={() => appendRealEstate({ fastanumer: '', heimilisfang: '', fasteign_mat: '' })}
              icon="add"
              iconType="outline"
              size="small"
            >
        ...
        {/* Vehicles Section */}
        <Box marginY={4}>
          <Text variant="h3" as="h2">Bifreiðir</Text>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Input
                        ...
                        errorMessage={errors.vehicles?.[index]?.kaupverd?.message}
                        required
                      />
                    )}
                  />
          ...
          <Box display="flex" justifyContent="flexEnd" marginTop={2}>
            <Button
              variant="ghost"
              onClick={() => appendVehicle({ numer: '', kaupar: '', kaupverd: '' })}
              icon="add"
              iconType="outline"
              size="small"
            >
