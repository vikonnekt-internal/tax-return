...
const debtExpenseSchema = z.object({
  ...
  upphæð: z.string().regex(/^\d+(\.\d+)?$/, { message: 'Upphæð verður að vera tala' })
});
...
const OtherDebtsExpenses: React.FC<OtherDebtsExpensesProps> = ({ onNext, initialData, onBack }) => {
  const {
    control,
    handleSubmit,
    formState: { errors }
          ...
          <Box key={field.id} marginY={2}>
            <GridRow>
              <GridColumn span={['12/12', '6/12', '6/12']}>
                <Box marginBottom={1}>
                  <Text variant="small" color="blue400">Tegund</Text>
                </Box>
                <Controller
                  ...
                  render={({ field }) => (
                    <Select
                      ...
                      errorMessage={errors.debtExpenses?.[index]?.tegund?.message}
                      required
                    />
                  )}
                />
              ...
              <GridColumn span={['12/12', '6/12', '6/12']}>
                <Box marginBottom={1}>
                  <Text variant="small" color="blue400">Upphæð</Text>
                </Box>
                <Controller
                  ...
                  render={({ field }) => (
                    <Input
                      ...
                      errorMessage={errors.debtExpenses?.[index]?.upphæð?.message}
                      required
                    />
                  )}
                />
